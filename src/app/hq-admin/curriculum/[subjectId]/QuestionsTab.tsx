...
function AiImportModal({ subject, topics, quizSets, lectures, onClose, onGenerate }: { subject: Subject; topics: Topic[]; quizSets: QuizSet[]; lectures: Lecture[]; onClose: () => void; onGenerate: (json: string) => void }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white border border-stone-200 rounded-3xl p-8 w-full max-w-2xl shadow-2xl space-y-5">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-lg font-black text-stone-800">✨ AI JSON Import</p>
                        <p className="text-xs font-bold text-stone-400 mt-0.5">Paste the JSON output from your AI tool directly</p>
                    </div>
                ...
                <div className="bg-stone-50 border border-stone-200 rounded-2xl p-4 space-y-1.5">
                    <p className="text-[10px] font-black uppercase tracking-widest text-stone-500">Required JSON fields per question</p>
                    <div className="flex flex-wrap gap-2">
                        {[
                            { f: "type", v: "\"cla\" | \"practice\" | \"exam\"" },
                            { f: "input_type", v: "\"mcq\" | \"text\"" },
                            { f: "module_from", v: "number" },
                            { f: "module_to", v: "number" },
                            { f: "difficulty", v: "\"easy\" | \"medium\" | \"hard\"" },
                            { f: "question", v: "string" },
                            { f: "options", v: "[A, B, C, D] — mcq only" },
                            { f: "correct_index", v: "0–3 — mcq only" },
                            { f: "lecture_id", v: "UUID — cla only" },
                        ].map(({ f, v }) => (
                            <div key={f} className="flex items-center gap-1.5 bg-white border border-stone-200 rounded-lg px-2 py-1">
                                <span className="text-[10px] font-black text-stone-700 font-mono">{f}</span>
                                <span className="text-[9px] text-stone-400 font-medium">{v}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <label className="text-[9px] font-black uppercase tracking-widest text-stone-500">Paste AI-generated JSON</label>
                        <button
                    ...
                    <textarea
                        ...
                        rows={12}
                        placeholder={'[\n  {\n    "type": "practice",\n    "input_type": "mcq",\n    "module_from": 1,\n    "module_to": 1,\n    "difficulty": "easy",\n    "question": "...",\n    "options": ["A","B","C","D"],\n    "correct_index": 0\n  }\n]'}
                        className="w-full bg-stone-50 border border-stone-200 focus:border-purple-300 focus:ring-2 focus:ring-purple-100 rounded-2xl px-4 py-3 text-xs font-mono text-stone-700 placeholder:text-stone-300 outline-none resize-none transition-all"
                    />
                </div>
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        onClick={() => { if (json.trim()) onGenerate(json); }}
                        disabled={!json.trim()}
                        className="flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black text-xs uppercase tracking-widest rounded-xl hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-sm"
                    >
...
function _Unused_AiImportModal_OLD({ subject, topics, quizSets, lectures, onClose, onGenerate }: { subject: Subject; topics: Topic[]; quizSets: QuizSet[]; lectures: Lecture[]; onClose: () => void; onGenerate: (json: string) => void }) {
    const [category, setCategory] = useState<"practice" | "lecture" | "concept" | "exam" | null>(null);
...
- Output MUST be valid JSON. No extra text.${transcriptSection}`;
                ...
                promptStr = `You are an expert curriculum designer. Your task is to convert and format the provided unstructured PYQ questions into our strictly formatted JSON array.
...
{`[
  {
    "type": "cla", // "cla", "practice", or "exam"
    "input_type": "mcq", // "mcq" or "text"
    "module_from": 1, // Start module
    "module_to": 1, // End module
    "lecture_id": "UUID", // Optional. Targeted lecture ID for "cla".
    "is_concept_builder": true, // Optional boolean for Concept Builder.
    "difficulty": "easy", // Optional. "easy", "medium", "hard".
    "question": "Question text...", 
    "options": ["A", "B", "C", "D"], // Omit if "text"
    "correct_index": 0, // 0-based index. Omit if "text"
    "explanation": "Optional explanation text",
    "quiz_set_id": "UUID", // Required if type is "exam"
    "word_limit": 250 // Required if input_type is "text"
  }
]`}
                        </pre>
                    </div>
...
export default function QuestionsTab({ subject, initialQuestions, topics, initialQuizSets, lectures = [] }: {
    subject: Subject; initialQuestions: Question[]; topics: Topic[]; initialQuizSets: QuizSet[]; lectures?: Lecture[];
}) {
