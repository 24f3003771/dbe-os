...
export default function NoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleAiRedirect = (ai: 'chatgpt' | 'gemini' | 'claude' | 'copy', content: string, title: string) => {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
                    <div className="bg-white rounded-3xl p-6 shadow-2xl max-w-sm w-full flex flex-col animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="flex flex-col gap-3">
                            <button 
                                onClick={() => handleAiRedirect('chatgpt', note.content, `${subjectTitle} - ${note.title}`)}
                            ...
                            <button 
                                onClick={() => handleAiRedirect('gemini', note.content, `${subjectTitle} - ${note.title}`)}
                            ...
                            <button 
                                onClick={() => handleAiRedirect('claude', note.content, `${subjectTitle} - ${note.title}`)}
                            ...
                            <button 
                                onClick={() => handleAiRedirect('copy', note.content, `${subjectTitle} - ${note.title}`)}
