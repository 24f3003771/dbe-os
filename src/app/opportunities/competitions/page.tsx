...
const CORPORATE_COMPS = [
    ...
    { id: "google-step", name: "Google HashCode", organizer: "Google", category: "Tech", portal: "https://codingcompetitions.withgoogle.com/hashcode", status: "Seasonal", industry: "Big Tech", tier: "T1" },
                ...
                <div className="flex items-center gap-2 bg-slate-100/50 p-1 rounded-2xl border border-slate-200/50">
                    {OPPORTUNITY_NAV.map((nav) => (
                        <Link 
                            key={nav.name}
                            href={nav.href}
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${nav.active ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'}`}
                        >
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-16">
                        <div className="space-y-8 max-w-3xl">
                            <div className="inline-flex items-center gap-2 bg-indigo-500/10 px-4 py-2 rounded-full text-indigo-400 font-black text-[10px] tracking-widest uppercase border border-indigo-500/20">
                ...
                <section className="max-w-7xl mx-auto px-6 -mt-20">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <button 
                            ...
                            className="group relative bg-white border border-stone-100 rounded-[3rem] p-12 text-left hover:border-indigo-500 transition-all shadow-2xl shadow-stone-200/50"
                        >
                            <div className="space-y-6">
                                <div className="w-20 h-20 bg-stone-50 rounded-[1.8rem] flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                                    <Medal className="w-10 h-10" />
                                </div>
                                <div className="space-y-2">
                                    <h3 className="text-4xl font-black font-headline tracking-tight leading-none italic">Elite Corporate <br/><span className="text-indigo-600">Challenges.</span></h3>
                                    <p className="text-stone-500 font-medium text-lg leading-relaxed">Direct path to HUL LIME, TAS, BCG Strategy Cup and more.</p>
                                </div>
                                <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-indigo-600">
                        ...
                        <button 
                            ...
                            className="group relative bg-indigo-950 rounded-[3rem] p-12 text-left hover:ring-2 hover:ring-indigo-500/50 transition-all shadow-2xl"
                        >
                            <div className="space-y-6">
                                <div className="w-20 h-20 bg-white/10 rounded-[1.8rem] flex items-center justify-center text-white group-hover:bg-indigo-500 transition-all">
                                    <Rocket className="w-10 h-10" />
                                </div>
                                <div className="space-y-2">
                                    <h3 className="text-4xl font-black font-headline tracking-tight leading-none italic text-white">Live Unstop <br/><span className="text-indigo-400">Hunter.</span></h3>
                                    <p className="text-indigo-200/70 font-medium text-lg leading-relaxed">Extract live competitions, hackathons, and quizzes directly from Unstop APIs.</p>
                                </div>
                                <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-white group-hover:text-indigo-400">
                                    Extract Events <ChevronRight className="w-4 h-4" />
                                </div>
                            </div>
                            <div className="absolute top-8 right-8">
                                <div className="px-3 py-1 bg-indigo-500 text-[8px] font-black uppercase tracking-widest text-white rounded-full animate-pulse">Sync Active</div>
                            </div>
                        </button>
                    </div>
                </section>
                ...
                <section className="max-w-7xl mx-auto px-6 space-y-10">
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 bg-white/80 backdrop-blur-md border border-slate-100 p-6 rounded-[3.5rem] shadow-2xl shadow-slate-200/40 sticky top-24 z-50">
                        <div className="relative flex-1 w-full">
                            ...
                            <input 
                                type="text"
                                placeholder={viewMode === 'PORTALS' ? "Find top challenges (HUL, TAS, Google)..." : "Describe the event (e.g. Case Study, Hackathon)..."}
                                className="w-full bg-slate-50 border-none rounded-[2rem] py-5 pl-16 pr-8 focus:ring-2 focus:ring-indigo-500/20 transition-all font-bold text-slate-700 text-lg placeholder:text-slate-300"
                        ...
                        {viewMode === 'LIVE' && (
                            <button 
                                ...
                                className="w-full md:w-auto bg-slate-900 text-white px-12 py-5 rounded-[2rem] font-black text-sm uppercase tracking-widest hover:bg-indigo-600 transition-all flex items-center gap-3 disabled:opacity-50 justify-center shadow-xl shadow-slate-900/10"
                            >
                                {isSearching ? <Loader2 className="w-5 h-5 animate-spin" /> : <Rocket className="w-5 h-5" />}
                                {isSearching ? "Searching Unstop..." : "Extract Events"}
                            </button>
                        )}
                    </div>
...
function RoadmapItem({ step, label, detail, icon: Icon, color }: any) {
    return (
        <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] space-y-4 hover:shadow-xl transition-all group">
            <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-slate-300 uppercase underline decoration-indigo-500/50 underline-offset-8 decoration-2">{step}</span>
                <div className={`w-12 h-12 rounded-2xl ${color} flex items-center justify-center text-white shadow-lg`}>
                    <Icon className="w-6 h-6" />
                </div>
            </div>
            <div>
                <h4 className="text-lg font-black font-headline text-slate-900 group-hover:text-indigo-600 transition-colors italic">{label}</h4>
                <p className="text-[11px] text-slate-500 font-medium leading-relaxed mt-2 italic">{detail}</p>
            </div>
        </div>
    );
}
...
function CorporateCompCard({ comp }: any) {
    return (
        <a href={comp.portal} target="_blank" className="group block h-full">
            <div className="h-full bg-white border border-slate-100 rounded-[2.5rem] p-10 flex flex-col justify-between hover:border-indigo-500/30 transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/5 relative overflow-hidden">
                <div className="space-y-8">
                    <div className="flex justify-between items-start">
                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner border border-slate-100">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.3em] mb-1">{comp.industry}</p>
                        <h3 className="text-2xl font-black font-headline text-slate-900 tracking-tighter italic leading-tight">{comp.name}</h3>
                        <p className="text-slate-500 text-[11px] font-bold mt-1">{comp.organizer}</p>
                    </div>
                </div>
                <div className="mt-12 flex items-center justify-between border-t border-slate-50 pt-8">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">View Details</span>
                    <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform shadow-sm group-hover:bg-indigo-50 group-hover:text-indigo-600">
                ...
                <div className="space-y-8">
                    <div className="flex justify-between items-start">
                        <div className="w-16 h-16 rounded-[1.4rem] flex items-center justify-center text-white group-hover:scale-105 transition-all shadow-lg border border-slate-100 overflow-hidden relative">
                            ...
                            <div className={`w-full h-full items-center justify-center bg-gradient-to-br font-black text-2xl ${['from-indigo-500 to-blue-600', 'from-emerald-500 to-teal-600', 'from-rose-500 to-pink-600', 'from-amber-500 to-orange-600', 'from-violet-500 to-purple-600'][comp.organizer?.length % 5]} ${comp.logo ? 'hidden' : 'flex'}`}>
                        ...
                        <div className="flex flex-col items-end gap-1.5">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                                <Clock className="w-3 h-3" /> {comp.daysLeft} days left
                            </span>
                        </div>
                    </div>
                    <div className="space-y-2">
                        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.3em]">{comp.category}</p>
                        <h3 className="text-xl font-black font-headline text-slate-900 tracking-tight leading-tight italic group-hover:text-indigo-600 transition-colors line-clamp-2">{comp.title}</h3>
                        <p className="text-slate-500 text-[11px] font-bold">{comp.organizer}</p>
                    </div>
                </div>
                <div className="mt-12 flex items-center justify-between border-t border-slate-50 pt-8">
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Apply Now</span>
                    <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform shadow-sm group-hover:bg-indigo-50 group-hover:text-indigo-600">
